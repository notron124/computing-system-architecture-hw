...
section .data
    filename db "output/output_broken.txt", 0
...
_start:
    ...
    mov rdi, rbx
    mov rsi, message
    mov rdx, 123                ; Если указать большую длину, он все равно
                                ; делает запись в файл со всеми данными из
                                ; секции .data (а то и дальше)
                                ; Это можно увидеть в output_broken.txt (make results)
                                ; Возможно ли этого как-то избежать, кроме
                                ; удаления файла в целом?
    syscall
