...
section .bss
    ...
    reversed_str resb 1024      ; 1KB массив для "перевернутой" строки
                                    ...
                                    ; По поводу проблемы с неизвестным разамером
                                    ; входных данных: в данной задаче
                                    ; оптимальнее всего использовать заведомо
                                    ; достаточный буффер, то-есть явно указать в
                                    ; условной документации максимальный размер
                                    ; строки. Если все же необходимо читать
                                    ; строку "бесконечной" (сколько памяти,
                                    ; столько и длина) длины то вот мои решения: 
                                    ; Решение №1: читать по 1 байту и сразу
                                    ; как-то обрабатывать, в частности
                                    ; реализовать LIFO, то-есть в итоге строку
                                    ; даже не нужно будет явно переворачивать
                                    ; между буфферами. 
                                    ; (Upd. Понял, что LIFO можно организовать, если пушить, а потом
                                    ; забирать из стека) 
                                    ; Решение №2: Читать "чанками" по n
                                    ; байт, при необходимости увеличивать
                                    ; буффер.
                                    ; (Второе решение - динамическя аллокация, опасно, сложно)
section .text
_start:
